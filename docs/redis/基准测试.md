...
> 注意：使用下面脚本测试报错，所以无法使用 redis-benchmark 测试 Redis 集群 Lua 脚本性能。使用 [示例](https://gitee.com/dexterleslie/demonstration/tree/main/demo-redis/demo-lettuce-benchmark) Lettuce JMH 测试。
...
6. 结论：通过 6 个控制台 QPS 输出计算得出单机版的 Redis QPS 约为 14w，被压测的 Redis 进程 CPU 利用率为 91%。
...
2. 运行 BenchmarkStandaloneTests 测试

   ```bash
   testBenchmarkByUsingAsyncApi 结果：2000000 次 set 耗时 6865 毫秒
   testBenchmarkByUsingSyncApi 结果：1000000 次 set 耗时 6860 毫秒
   ```

3. 结论：Lettuce 客户端同步和非同步 api 性能都很高。
...
2. 运行 BenchmarkClusterTests 测试

   ```bash
   testBenchmarkByUsingAsyncApi 结果：2000000 次 set 耗时 3199 毫秒
   testBenchmarkByUsingSyncApi 结果：1000000 次 set 耗时 3517 毫秒
   ```

3. 结论：Lettuce 客户端同步和非同步 api 性能都很高，集群 Redis 写性能也很高。
